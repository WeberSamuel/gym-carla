...
class CarlaEnv(gym.Env):
  ...
  def __init__(self, params):
    ...
    self._set_action_and_obs_spaces(params)

    # Connect to carla server and get world object
    client = self._connect_carla(params)
    self._load_world(params, client)

    # Get spawn points
    self._load_spawn_points()

    self._load_blueprints(params)
  ...
  def _load_world(self, params, client):
      self.world = client.load_world(params['town'])
      print('Carla server connected!')
      # Set weather
      self.world.set_weather(carla.WeatherParameters.ClearNoon)

  def _connect_carla(self, params):
      print('connecting to Carla server...')
      client = carla.Client('localhost', params['port'])
      client.set_timeout(10.0)
      return client

  def _set_action_and_obs_spaces(self, params):
  ...
  def reset(self):
    ...
    self._spawn_environment_cars()

    # Spawn pedestrians
    self._spawn_pedestrians()

    # Get actors polygon list
    self._init_actor_polygons()
  ...
  def _spawn_environment_cars(self):
  ...
  def _update_actor_polygons(self):
      vehicle_poly_dict = self._get_actor_polygons('vehicle.*')
      self.vehicle_polygons.append(vehicle_poly_dict)
  ...
  def _get_obs(self):
    ...
    lidar = self._get_lidar_obs(birdeye)
    ...
    if self.pixor:
      pixor_obs = self._get_pixor_obs(birdeye)
    ...
    if self.pixor:
      obs.update(pixor_obs)

    return obs

  def _get_pixor_obs(self, birdeye):
      ego_x, ego_y, ego_yaw, speed = self._get_car_position_properties()
      ...
      roadmap = self._get_roadmap(birdeye)
  ...
  def _get_car_position_properties(self):
      ...
      return ego_x,ego_y,ego_yaw,speed

  def _get_camera_obs(self):
      camera = resize(self.camera_img, (self.obs_size, self.obs_size)) * 255
      return camera

  def _get_lidar_obs(self, birdeye):
      point_cloud = []
  ...
  def _get_reward(self):
    ...
    v = self._get_speed_reward()
    
    # reward for collision
    r_collision = self._get_collision_reward()

    # reward for steering:
    r_steer = self._get_steer_reward()

    # reward for out of lane
    ego_x, ego_y = get_pos(self.ego)
    dis, w = get_lane_dis(self.waypoints, ego_x, ego_y)
    r_out = self._get_out_of_lane_reward(dis)
    ...
    # cost for too fast
    r_fast = self._get_over_speed_limit_reward(lspeed_lon)

    # cost for lateral acceleration
    r_lat = self._get_lateral_acceleration_reward(lspeed_lon)

    r = self._get_total_reward(r_collision, r_steer, r_out, lspeed_lon, r_fast, r_lat)

    return r

  def _get_total_reward(self, r_collision, r_steer, r_out, lspeed_lon, r_fast, r_lat):
      return 200*r_collision + 1*lspeed_lon + 10*r_fast + 1*r_out + r_steer*5 + 0.2*r_lat - 0.1

  def _get_lateral_acceleration_reward(self, lspeed_lon):
      return - abs(self.ego.get_control().steer) * lspeed_lon**2

  def _get_over_speed_limit_reward(self, lspeed_lon):
      r_fast = 0
      if lspeed_lon > self.desired_speed:
        r_fast = -1
      return r_fast

  def _get_out_of_lane_reward(self, dis):
      r_out = 0
      if abs(dis) > self.out_lane_thres:
        r_out = -1
      return r_out

  def _get_steer_reward(self):
      return -self.ego.get_control().steer**2
